...
5. **The composition roots are still too broad.** `src/index.ts` and the
   ...
   overview, Settings logs, API keys, and the Settings page shell out of
   `App.tsx`; runtime Settings composition moved out of `src/index.ts`.
...
- incremental Console decomposition for Settings data loading, navigation,
  editor pages, logs, API keys, overview, and shell layout.
