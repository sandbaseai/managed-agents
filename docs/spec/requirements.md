...
**一句话定位**：Open-source Claude Managed Agents-style runtime — run stateful agents locally with any model, a minimal built-in dashboard, and scenario templates.
...
| Dashboard | 他们的界面 | **v1 内嵌最小 Dashboard；品牌化 Console 延后** |
...
4. 不想从零搭建 → 需要现成方案模板
5. 要给同事/老板展示 → v1 需要可用的本地 Dashboard，完整品牌化 Console 延后
6. 要对接现有系统 → 需要标准 API
...
| **managed-agents** | 简洁有力，工具箱意象 | 和 CMA 关联需要靠 tagline |
...
**推荐名字**：**managed-agents** 或 **AgentForge**

Description 这样写：
> `Open-source local runtime inspired by Claude Managed Agents (CMA). Multi-agent orchestration, MCP tools, scenario templates, minimal built-in dashboard. Any model — run Ollama/vLLM/Claude/GPT locally. CMA-style API.`

### 核心卖点（GitHub README 第一屏）

```
🚀 1 分钟启动 — npx managed-agents start，零配置运行多 Agent 系统
📦 场景模板 — 客服、编程、研究、运维...选模板即有完整方案
🧠 Any Model — Ollama/vLLM/Claude/GPT，本地或云端自由切换
🔌 MCP + Skills — 标准协议扩展，对接任何外部工具
🎨 Dashboard — v1 内嵌最小界面，对话/Session/事件流可观察；完整 Console 延后
...
| **独立开发者** | 想在自己项目里嵌入 Agent | 启动 managed-agents → 用 API 对接 → 本地 Ollama 省钱 |
| **技术经理** | 评估 AI Agent 方案 | 打开 Dashboard 演示 → 看到效果 → 决定落地 |
| **DevOps** | 部署到生产 | 同一套 YAML → docker compose up → 或推云端 |

### 项目名候选

- **managed-agents** — 简洁有力 + "Run Claude Managed Agents locally" tagline 蹭热度
- 备选：CMA-Local / LocalCMA / managed-agents-CMA
- npm 包名：`managed-agents`
- GitHub 仓库名：`managed-agents` (org: sandbaseai)
...
**技术栈**：v1 使用内嵌 HTML/CSS/JS，减少依赖与发布复杂度；React 19 + Vite + Tailwind v4 + shadcn/ui + TanStack Query + React Router 作为 v1.1/v2 Console 方向。

**页面结构参考**（从 OMA 提炼适合我们的）：

| OMA 页面 | 我们是否需要 | 对应需求 |
|----------|------------|---------|
| Dashboard | v1 最小版；v1.1/v2 完整版 | 首页概览 |
...
- v1 不引入前端构建链；v1.1/v2 使用 shadcn/ui 组件库（和 OMA 一致）
...
|---|---|---|---|---|
| 部署 | Anthropic 云端 | CF Workers / Docker | Docker + Redis + PG | **`npx managed-agents start`** |
...
1. THE Platform SHALL 支持通过 `npx managed-agents start` 命令以单进程方式启动全部服务（HTTP API 服务器、Web UI 静态资源服务、Agent 运行时）
2. THE Platform SHALL 在启动时自动初始化本地 SQLite 数据库文件（默认路径为项目目录下的 `.managed-agents/data.db`），无需用户手动配置数据库
...
3. THE Agent_Runtime SHALL 支持在项目级配置文件（`managed-agents.config.yaml`）中定义模型注册表，每个模型条目包含：`name`（引用名）、`provider`（提供者类型）、`model`（模型标识）、`base_url`（API 端点，可选）、`api_key`（认证密钥，支持环境变量引用）
...
**v1 范围决策：** first public release 只要求内嵌最小 Dashboard。完整 React/Vite/shadcn Console 是 v1.1/v2 目标。
...
5. THE Web_UI SHOULD 提供模型配置页面，展示当前模型注册表内容，支持在线添加和修改模型配置（修改后持久化到配置文件）。该能力可延后到完整 Console
6. WHILE Agent 执行工具调用时，THE Web_UI SHALL 在对话界面实时展示工具调用的名称、参数和返回结果
7. THE Web_UI SHALL 在 v1 使用内嵌静态 HTML/CSS/JS，由 Hono 服务器托管；完整 React/Vite Console SHALL 作为 v1.1/v2 目标
...
1. THE CLI SHALL 提供 `init` 命令，在当前目录生成项目脚手架（包含 `agents/` 目录、`skills/` 目录、`managed-agents.config.yaml` 配置文件模板和示例 Agent 定义文件）
...
2. THE Agent_Runtime SHALL 提供默认的 `local` Sandbox_Provider 实现，使用本地子进程（child_process）执行命令，工作目录为项目目录下的 `.managed-agents/sandbox/<session_id>/`
